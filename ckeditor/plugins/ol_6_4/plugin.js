$(function(){
    ...
    var action_6_4= {
        exec:function(editor){
            ...
            //判斷是否為ctrl+a
            if(editorcontent.find('p').has('span').length == 0 && editorcontent.find('ol').has('span').length == 0){
                editorcontent.find('span').first().after('<ol class="labelall[1]+symbolall[1] start4 add"></ol>');
                for(var i=0;i<text.length;i++){
                ...
                if(editorcontent.find('ol').has('span').length > 0){
                    ...
                    if(editorcontent.find('.remove').is('p') || isol > 1){
                        editorcontent.find('ol').has('span').first().attr('class','labelall[1]+symbolall[1] start4 add');
                    ...
                    //如果在OL內就只改標號不產生
                        ...
                        editorcontent.find('span').parent().parent().attr('class','labelall[1]+symbolall[1] start4 add');    
                    }
                }
                //沒標號
                else{
                    editorcontent.find('ol').removeClass('remove');
                    editorcontent.find('p').has('span').first().after('<ol class="labelall[1]+symbolall[1] start4 add"></ol>');    
                    for(var i=0;i<text.length;i++){
                        editorcontent.find('.add').append(text[i]);
                    }
                    editorcontent.find('.remove').remove();
    ...
    //Section 2 : 建立按鈕
    name_6_4='NewButton_6_4';
    CKEDITOR.plugins.add('ol_6_4',{
        init:function(editor){
            editor.addCommand(name_6_4,action_6_4);
            editor.ui.addButton('ol_6_4',{
                label:'6_4',
                icon: this.path + '(丁).ico',
                command:name_6_4
            });
        }        
    });
