$(function(){
    ...
    var action_capital_cnnum_2= {
        exec:function(editor){
            ...
            //判斷是否為ctrl+a
            if(editorcontent.find('p').has('span').length == 0 && editorcontent.find('ol').has('span').length == 0){
                editorcontent.find('span').first().after('<ol class="labelall[3]+symbolall[0] start2 add"></ol>');
                for(var i=0;i<text.length;i++){
                ...
                if(editorcontent.find('ol').has('span').length > 0){
                    ...
                    if(editorcontent.find('.remove').is('p') || isol > 1){
                        editorcontent.find('ol').has('span').first().attr('class','labelall[3]+symbolall[0] start2 add');
                    ...
                    //如果在OL內就只改標號不產生
                        ...
                        editorcontent.find('span').parent().parent().attr('class','labelall[3]+symbolall[0] start2 add');    
                    }
                }
                //沒標號
                else{
                    editorcontent.find('ol').removeClass('remove');
                    editorcontent.find('p').has('span').first().after('<ol class="labelall[3]+symbolall[0] start2 add"></ol>');    
                    for(var i=0;i<text.length;i++){
                        editorcontent.find('.add').append(text[i]);
                    }
                    editorcontent.find('.remove').remove();
    ...
    //Section 2 : 建立按鈕
    name_capital_cnnum_2='NewButton_capital_cnnum_2';
    CKEDITOR.plugins.add('capital_cnnum_2',{
        init:function(editor){
            editor.addCommand(name_capital_cnnum_2,action_capital_cnnum_2);
            editor.ui.addButton('capital_cnnum_2',{
                label:'capital_cnnum_2',
                            //icon: this.path + '貳、.ico',
                            command:name_capital_cnnum_2
                        });
        }        
    });
