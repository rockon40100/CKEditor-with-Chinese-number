$(function(){
    ...
    var action_nestnum= {
        exec:function(editor){  
            ...
            var str = prompt('請在輸入框輸入起始編號，例如：1.2，2.1.3......','');
            //只能輸入數字+點+數字
            if(str != null && !/^[1-9]+([.]{1}[1-9]?[1-9]?)+$/.test(str)){
                alert("輸入不合格式");
                str = null;
            }
            //如果輸入兩個點
            ...
            if(str != null){
                ...
                var sym = '';
                if(strsplit[strlength-1] != ''){
                    for(i=0;i<strlength-1;i++){
                        sym += strsplit[i]+'.';
                    }
                }
                else{
                    strsplit[strlength-1] = strsplit[0];
                }
                var lab = labelall[4];
                ...
                //判斷是否為ctrl+a
                if(editorcontent.find('p').has('span').length == 0 && editorcontent.find('ol').has('span').length == 0){
                    editorcontent.find('span').first().after('<ol class="labelall[4]+'+sym+' start'+strsplit[strlength-1]+' add"></ol>');
                    for(var i=0;i<text.length;i++){
                    ...
                    if(editorcontent.find('ol').has('span').length > 0){
                        ...
                        if(editorcontent.find('.remove').is('p') || isol > 1){
                            editorcontent.find('ol').has('span').first().attr('class','labelall[4]+'+sym+' start'+strsplit[strlength-1]+' add');
                        ...
                        //如果在OL內就只改標號不產生
                            ...
                            editorcontent.find('span').parent().parent().attr('class','labelall[4]+'+sym+' start'+strsplit[strlength-1]+' add');    
                        }
                    }
                    //沒標號
                    else{
                        editorcontent.find('ol').removeClass('remove');
                        editorcontent.find('p').has('span').first().after('<ol class="labelall[4]+'+sym+' start'+strsplit[strlength-1]+' add"></ol>');    
                        for(var i=0;i<text.length;i++){
                            editorcontent.find('.add').append(text[i]);
                        }
                        editorcontent.find('.remove').remove();
    ...
    //Section 2 : 建立按鈕
    name_nestnum='NewButton_nestnum';
    CKEDITOR.plugins.add('nestnum',{
        init:function(editor){
            editor.addCommand(name_nestnum,action_nestnum);
            editor.ui.addButton('nestnum',{
                label:'nestnum',
                icon: this.path + '編號.png',
                command:name_nestnum,
