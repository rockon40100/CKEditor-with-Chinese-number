$(function(){
    ...
    var action_7_12= {
        exec:function(editor){
            ...
            //判斷是否為ctrl+a
            if(editorcontent.find('p').has('span').length == 0 && editorcontent.find('ol').has('span').length == 0){
                editorcontent.find('span').first().after('<ol class="labelall[2]+symbolall[0] start12 add"></ol>');
                for(var i=0;i<text.length;i++){
                ...
                if(editorcontent.find('ol').has('span').length > 0){
                    ...
                    if(editorcontent.find('.remove').is('p') || isol > 1){
                        editorcontent.find('ol').has('span').first().attr('class','labelall[2]+symbolall[0] start12 add');
                    ...
                    //如果在OL內就只改標號不產生
                        ...
                        editorcontent.find('span').parent().parent().attr('class','labelall[2]+symbolall[0] start12 add');    
                    }
                }
                //沒標號
                else{
                    editorcontent.find('ol').removeClass('remove');
                    editorcontent.find('p').has('span').first().after('<ol class="labelall[2]+symbolall[0] start12 add"></ol>');    
                    for(var i=0;i<text.length;i++){
                        editorcontent.find('.add').append(text[i]);
                    }
                    editorcontent.find('.remove').remove();
    ...
    //Section 2 : 建立按鈕
    name_7_12='NewButton_7_12';
    CKEDITOR.plugins.add('ol_7_12',{
        init:function(editor){
            editor.addCommand(name_7_12,action_7_12);
            editor.ui.addButton('ol_7_12',{
                label:'7_12',
                icon: this.path + '亥、.ico',
                command:name_7_12
            });
        }        
    });
